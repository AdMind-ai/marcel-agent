---
sidebar_position: 1
title: "Marcel Agent Quickstart"
description: "Your first conversation with Marcel Agent — from install to chatting in under 5 minutes"
---

# Marcel Agent Quickstart

This guide gets you from zero to a working Marcel setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Marcel Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Marcel Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Marcel for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Marcel working on my machine | `marcel setup` | Run a real chat and verify it responds |
| I already know my provider | `marcel model` | Save the config, then start chatting |
| I want a bot or always-on setup | `marcel gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `marcel model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `marcel model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Marcel cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `marcel model` to walk through the choice interactively:
...
- **Quick setup** — sign in through Nous Portal for optional OAuth access, no API keys; sets up a model plus the Tool Gateway tools.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `marcel update`. Re-enable anything later with `marcel tools`, seed skills with `marcel skills opt-in --sync`, or tune settings with `marcel setup agent`.
:::
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `marcel_cli/models.py` may change between releases) | `marcel model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `marcel model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `marcel model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `marcel model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `marcel model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `marcel model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Marcel separates secrets from normal config:

- **Secrets and tokens** → `~/.marcel/.env`
- **Non-secret settings** → `~/.marcel/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
marcel config set model anthropic/claude-opus-4.6
marcel config set terminal.backend docker
marcel config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
marcel            # classic CLI
marcel --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Marcel ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `marcel` vs `marcel --tui`.
:::
...
```bash
marcel --continue    # Resume the most recent session
marcel -c            # Short form
```
...
```bash
marcel gateway setup    # Interactive platform configuration
```
...
- `marcel tools` — tune tool access per platform
- `marcel skills` — browse and install reusable workflows
...
```bash
marcel config set terminal.backend docker    # Docker isolation
marcel config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `marcel egress setup && marcel egress start`; `marcel setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Marcel install directory (the curl installer placed it at
# ~/.marcel/marcel-agent on Linux/macOS or %LOCALAPPDATA%\marcel\marcel-agent on Windows):
cd ~/.marcel/marcel-agent
...
Skills are on-demand instruction documents that teach Marcel how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Marcel ships with a catalog of bundled skills already installed in `~/.marcel/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
marcel skills browse                      # list everything available
marcel skills search kubernetes           # find skills by keyword
marcel skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `marcel skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Marcel ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.marcel/marcel-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Marcel opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `marcel model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `marcel gateway setup` and check `marcel gateway status` |
| `marcel --continue` can't find old session | Switched profiles or session never saved | Check `marcel sessions list` and confirm you're in the right profile |
...
| `marcel doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `marcel model`
3. `marcel setup`
4. `marcel sessions list`
5. `marcel --continue`
6. `marcel gateway status`
...
| Command | Description |
|---------|-------------|
| `marcel` | Start chatting |
| `marcel model` | Choose your LLM provider and model |
| `marcel tools` | Configure which tools are enabled per platform |
| `marcel setup` | Full setup wizard (configures everything at once) |
| `marcel doctor` | Diagnose issues |
| `marcel update` | Update to latest version |
| `marcel gateway` | Start the messaging gateway |
| `marcel --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-marcel-to-another-machine)** — `marcel backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
