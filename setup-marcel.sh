...
# 5. Symlinks the 'marcel' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/marcel-uv-install.$$.log")"
...
if is_termux; then
    # Marcel currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Marcel requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink marcel into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up marcel command..."

MARCEL_BIN="$SCRIPT_DIR/venv/bin/marcel"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MARCEL_BIN" "$COMMAND_LINK_DIR/marcel"
echo -e "${GREEN}✓${NC} Symlinked marcel → $COMMAND_LINK_DISPLAY_DIR/marcel"
...
# ============================================================================
# Seed bundled skills into ~/.marcel/skills/
# ============================================================================

MARCEL_SKILLS_DIR="${MARCEL_HOME:-$HOME/.marcel}/skills"
mkdir -p "$MARCEL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.marcel/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MARCEL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  marcel status        # Check configuration"
if is_termux; then
    echo "  marcel gateway       # Run gateway in foreground"
else
    echo "  marcel gateway install # Install gateway service (messaging + cron)"
fi
echo "  marcel cron list     # View scheduled jobs"
echo "  marcel doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m marcel_cli.main setup
fi
