import { Box, Text, useStdout } from '@marcel/ink'
...
const TAG_FULL = 'An autonomous AI agent framework by AdMind.'
const TAG_MID = 'Autonomous AI agent framework · AdMind'
const TAG_TINY = 'Marcel · AdMind'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // marcel_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Upstream: Nous Research</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'marcel update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
