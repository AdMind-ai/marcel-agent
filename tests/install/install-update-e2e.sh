...
#
# Installs a real, earlier Marcel the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `marcel`.
#
...
# both together.
UPSTREAM_URL="${MARCEL_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
LOG_DIR="${MARCEL_E2E_LOG_DIR:-$(mktemp -d -t marcel-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED marcel accept FLAG on `marcel update`?
#
...
# that silently rots. `marcel update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "marcel update --help 2>&1" | grep -qF -- "$flag"
}
...
require_marcel_works() {
  local when="$1" out
  out="$(in_sandbox "marcel --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "marcel --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "marcel runs $when"
}

# ── install the earlier Marcel ─────────────────────────────────────────────
...
require_marcel_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: marcel update'
    ...
    # marcel which it accepts; older ones read the prompt from stdin, so close it.
    ...
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      ...
      fail "marcel update failed ($update_cmd)"
    fi
    require_landed_on_target 'marcel update'
    require_marcel_works 'after marcel update'
    ;;
  installer)
