"""Streamlined setup flows: the Nous Portal one-shot (`marcel portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``marcel_cli.setup`` take effect."""
...
logger = logging.getLogger("marcel_cli.setup")
...
def _blank_slate_done(config: dict, marcel_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from marcel_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         marcel skills opt-in --sync",
          "  Add MCP servers:     marcel mcp add", *extra, "  Tune agent settings: marcel setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from marcel_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``marcel model``. False when cancelled or failed (the
    message is already printed)."""
    from marcel_cli.setup import _info
    try:
        from marcel_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("  You can retry later with `marcel portal`.")

    if not _run_nous_flow(config, context="`marcel portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `marcel portal`."),
    ...
    _info("  Run `marcel portal info` to inspect routing.", "  Run `marcel` to start chatting.")
...
def _run_first_time_quick_setup(config: dict, marcel_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'marcel setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `marcel import`).
        from marcel_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    marcel setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  marcel setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Marcel.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Marcel update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``marcel-agent`` skill
    ...
    try:
        ...
        from marcel_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "marcel-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("marcel-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``marcel setup agent``."""
...
def _run_blank_slate_setup(config: dict, marcel_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `marcel-agent` skill is always kept so the agent",
          "can help you drive and configure Marcel itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, marcel_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `marcel update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, marcel_home, "  Enable tools:        marcel tools", "  Enable plugins:      marcel plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, marcel_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `marcel-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`marcel update` runs from re-injecting them. Opt back in any",
              "time with `marcel skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `marcel tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `marcel plugins list` / `marcel plugins install`.",
         "No plugins enabled. Add later with `marcel plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `marcel mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `marcel mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, marcel_home, "  Enable more tools:   marcel tools")


def _run_quick_setup(config: dict, marcel_home):
    ...
    from marcel_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'marcel setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Marcel to messaging apps to chat from anywhere.",
              "You can configure these later with 'marcel setup gateway'.")
