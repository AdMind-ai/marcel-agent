...
resolved through the module object so test patches on ``marcel_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from marcel_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'marcel setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'marcel setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'marcel tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("marcel setup", "          Re-run the full wizard"), ("marcel setup model", "    Change model/provider"),
    ("marcel setup terminal", " Change terminal backend"), ("marcel setup gateway", "  Configure messaging"),
    ("marcel setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("marcel config", "         View current settings"), ("marcel config edit", "    Open config in your editor"),
    ("marcel config set <key> <value>", ""))
_READY_ROWS = (
    ("marcel", "              Start chatting"), ("marcel gateway", "      Start messaging gateway"),
    ("marcel doctor", "       Check for issues"))
...
def _first_available_plugin_provider(registry: str, skip: str = None):
    ...
    try:
        import importlib
        from marcel_cli.plugins import _ensure_plugins_discovered
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'marcel setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `marcel tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'marcel setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via marcel auth spotify — check auth.json, not env vars
    try:
        from marcel_cli.auth import get_provider_auth_state
...
def _print_setup_summary(config: dict, marcel_home):
    """Print the setup completion summary."""
    from marcel_constants import display_marcel_home as _dhh
    ...
    try:
        from marcel_cli.auth import resolve_provider
        resolve_provider()
    except Exception:
        print()
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'marcel setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {marcel_home}/cron/, sessions/, logs/")):
...
import marcel_cli.setup as _setup  # noqa: E402  (bottom: marcel_cli.setup imports this module)
