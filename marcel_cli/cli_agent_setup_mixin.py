"""Agent construction + session-resume display for ``MarcelCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``marcel chat -q``.
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``MarcelCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        from marcel_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
        _primary_exc = None
        runtime = None
        try:
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'marcel model' to choose a provider, or "
                      "'marcel setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: marcel setup")
            return False
        ...
        # is just the provider slug/display name (`marcel chat --model <provider-name>`
        ...
        # Still empty (e.g. `marcel auth add` without `marcel model`): fall back to the
        # provider's first catalog model so the API doesn't reject an empty model.
        if not self.model and resolved_provider:
            try:
                from marcel_cli.models import get_default_model_for_provider
    ...
    def _resolve_fallback_runtime(self, primary_exc):
        ...
        from marcel_cli.auth import AuthError
        from marcel_cli.runtime_provider import resolve_runtime_provider
        ...
        for _fb in _fb_chain:
            ...
            if not _fb_provider or not _fb_model:
                continue
            try:
                from marcel_cli.fallback_config import resolve_entry_api_key
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``marcel model`` so onboarding has a single
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'marcel model' or 'marcel setup' any time.")
            return False
        try:
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'marcel model' to retry.")
        return False
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        from marcel_cli.models import resolve_fast_mode_overrides
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(marcel chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (marcel sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if not self._ensure_runtime_credentials():
            return False
        from marcel_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ensure_mcp_discovery_before_agent_build(
            logger=logger, single_query=getattr(self, "_single_query_mode", False))
        if self._session_db is None:
            try:
                from marcel_state import SessionDB
        ...
        try:
            ...
            from marcel_constants import partial_update_hint
    ...
    def _resume_history_limit_error(self, tip_only: bool = False):
        ...
        from marcel_state import SessionResumeTooLargeError
        try:
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (marcel sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ Marcel: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ Marcel: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
