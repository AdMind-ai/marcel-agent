"""Interactive setup wizard for Marcel Agent (config lives in ~/.marcel/).
...
``marcel_cli.setup.<name>`` keep working.
"""
...
from marcel_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_marcel_home is
# imported lazily at call sites (stale-module safety during marcel update).
from marcel_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_marcel_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_marcel_home,
)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Marcel Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    _info("The interactive wizard cannot be used here.", None,
          "Configure Marcel using environment variables or config commands:",
          "  marcel config set model.provider custom",
          "  marcel config set model.base_url http://localhost:8080/v1",
          "  marcel config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'marcel setup' in an interactive terminal to use the full wizard.", None)
...
def prompt(question: str, default: str = None, password: bool = False) -> str:
    ...
    try:
        if password:
            ...
            from marcel_cli.cli_output import line_input
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("marcel_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from marcel_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``marcel model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from marcel_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``MARCEL_NONINTERACTIVE=1`` (``marcel_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("MARCEL_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``MARCEL_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from marcel_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'marcel setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``marcel model`` flow (one code
    ...
    from marcel_cli.config import load_config, save_config
    print_header("Inference Provider")
    _info("Choose how to connect to your main chat model.",
          f"   Guide: {_DOCS_BASE}/integrations/providers", None)
    from marcel_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: marcel model")
    ...
    # keep safe defaults (configure via `marcel auth add` / `marcel setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into MARCEL_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("MARCEL_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          ...
          "  Run `marcel setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.marcel/logs/tool_calls.log (gateway only)",
)
_SESSION_RESET_HELP = (
...
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives MARCEL_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`marcel setup tools` == `marcel tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from marcel_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from marcel_cli.observability.shared_metrics import SharedMetricsStore
        from marcel_cli.observability.shared_metrics_sender import reconcile_send_consent
        from marcel_cli.sqlite_util import write_txn
...
# marcel_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from marcel_cli.setup_tts import setup_tts  # noqa: E402
from marcel_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from marcel_cli.setup_platforms import setup_gateway  # noqa: E402
from marcel_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from marcel_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from marcel_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
from marcel_cli.setup_marcel import setup_marcel  # noqa: E402


# ── Main Wizard Orchestrator ──

SETUP_SECTIONS = [
    ("marcel", "Marcel Agent", setup_marcel),
...
def _run_setup_section(config: dict, section: str) -> None:
    """``marcel setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    brand = "Marcel" if os.path.basename(sys.argv[0]).lower().startswith("marcel") else "Marcel"
    _print_banner(f"│     ⚕ {brand} Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, marcel_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {marcel_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'marcel config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from marcel_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``marcel setup <section>`` from SETUP_SECTIONS."""
    from marcel_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, marcel_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have Marcel configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'marcel setup model|terminal|",
    ...
    _print_setup_summary(config, marcel_home)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('marcel_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('marcel_constants', 'get_optional_skills_dir'),
