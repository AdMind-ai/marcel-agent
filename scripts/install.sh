...
# Python-driven tool session (e.g. Marcel terminal tool). A pre-set PYTHONPATH
...
if [ -n "${MARCEL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MARCEL_INSTALL_DIR"
...
#   code at /usr/local/lib/marcel-agent, command at /usr/local/bin/marcel,
#   data still at /root/.marcel (MARCEL_HOME).  Matches Claude Code / Codex CLI
#   and keeps Docker bind-mounted /root/ volumes lean.
ROOT_FHS_LAYOUT=false
DETECTED_BROWSER_EXECUTABLE=""

# Migrate before any installer stage can create MARCEL_HOME (node, config,
...
migrate_legacy_home_before_install() {
    ...
    command -v python3 >/dev/null 2>&1 || {
        log_error "Python is required for the pre-install legacy migration."
        return 1
    }
    MARCEL_HOME="$MARCEL_HOME" PYTHONPATH="$root${PYTHONPATH:+:$PYTHONPATH}" \
        python3 -m marcel_migration --destination "$MARCEL_HOME" || {
        log_error "Legacy home migration failed; refusing to continue installation."
        return 1
    }
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$MARCEL_HOME/.no-bundled-skills so future"
            echo "                   'marcel update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Marcel.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.marcel/marcel-agent"
            echo "                   default (root, Linux): /usr/local/lib/marcel-agent"
            echo "  --marcel-home PATH  Data directory (default: ~/.marcel, or \$MARCEL_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Marcel installs the code under"
            echo "  /usr/local/lib/marcel-agent and links the command into"
            echo "  /usr/local/bin/marcel (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MARCEL_HOME"
            echo "  (default /root/.marcel).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MARCEL_HOME/marcel-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `marcel update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Marcel-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Marcel Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install marcel command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `marcel` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MARCEL_HOME/marcel-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MARCEL_HOME/marcel-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/marcel-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MARCEL_HOME/marcel-agent — then preserve it)
#
# Always no-op when the user set --dir or $MARCEL_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MARCEL_HOME.
    if is_termux; then
        INSTALL_DIR="$MARCEL_HOME/marcel-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MARCEL_HOME/marcel-agent/.git" ]; then
            INSTALL_DIR="$MARCEL_HOME/marcel-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/marcel-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/marcel-agent"
        ...
        # /usr/local/bin/marcel wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/marcel"
        log_info "  Data:    $MARCEL_HOME (unchanged)"
...
# Point a Marcel-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $MARCEL_HOME/node/bin — which is NOT on
...
# Marcel's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Marcel-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$MARCEL_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$MARCEL_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$MARCEL_HOME/node/etc/npmrc"
}

get_marcel_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/marcel" ]; then
        echo "$link_dir/marcel"
    else
        echo "marcel"
    fi
}
...
install_uv() {
    ...
    # Marcel owns its own uv at $MARCEL_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (marcel_cli/managed_uv.py) looks in the same
    # place, so install.sh and `marcel update` stay in sync.
    local _managed_uv="$MARCEL_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $MARCEL_HOME/bin ..."
    mkdir -p "$MARCEL_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/marcel-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/marcel-uv-installer.$$.sh")"
    ...
    # directly into $MARCEL_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$MARCEL_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Marcel currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Marcel requires Python >=3.11,<3.14"
...
# replaced with the Marcel-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Marcel-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Marcel-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Marcel-managed Node from a previous run over a too-old system one.
    if [ -x "$MARCEL_HOME/node/bin/node" ] && [ -x "$MARCEL_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$MARCEL_HOME/node/bin/node" --version)"; then
        export PATH="$MARCEL_HOME/node/bin:$PATH"
        log_success "Node.js $("$MARCEL_HOME/node/bin/node" --version) found (Marcel-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Marcel-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Marcel requires Node 22.22+, 24.11+, or 26+) — installing Marcel-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.marcel/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.marcel/node/..."
    ...
    # Place into ~/.marcel/node/ and symlink binaries into the same bin dir
    # the marcel command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$MARCEL_HOME/node"
    mkdir -p "$MARCEL_HOME"
    mv "$extracted_dir" "$MARCEL_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$MARCEL_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$MARCEL_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$MARCEL_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$MARCEL_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.marcel/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Marcel install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Marcel Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `marcel update` path
                # (#4735).
                ...
                local stash_name
                stash_name="marcel-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``marcel update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Marcel behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"marcel-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up marcel command..."

    if [ "$USE_VENV" = true ]; then
        MARCEL_BIN="$INSTALL_DIR/venv/bin/python"
        MARCEL_ENTRYPOINT="$INSTALL_DIR/marcel"
    else
        MARCEL_BIN="$(which marcel 2>/dev/null || echo "")"
        if [ -z "$MARCEL_BIN" ]; then
            log_warn "marcel not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$MARCEL_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$MARCEL_ENTRYPOINT" ]; }; then
        log_warn "Marcel launcher prerequisites not found"
    ...
    # Create a user-facing shim for the marcel command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $MARCEL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$MARCEL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/marcel"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/marcel" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MARCEL_BIN" "$MARCEL_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/marcel" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/marcel"
    log_success "Installed marcel launcher → $command_link_display_dir/marcel"

    # Also expose `marcel-agent`. The `marcel-agent` console script declared in
    ...
    rm -f "$command_link_dir/marcel-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/marcel-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MARCEL_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/marcel-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MARCEL_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/marcel-agent"
    log_success "Installed marcel-agent launcher → $command_link_display_dir/marcel-agent"

    # Also expose `marcel-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `marcel-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Marcel as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/marcel-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/marcel-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MARCEL_BIN" "$MARCEL_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/marcel-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/marcel-acp"
    log_success "Installed marcel-acp launcher → $command_link_display_dir/marcel-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "marcel command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v marcel' \
                >/dev/null 2>&1; then
        ...
        log_info "marcel not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Marcel Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Marcel Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Marcel Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so marcel works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.marcel directory structure (config at top level, code in subdir)
    mkdir -p "$MARCEL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.marcel/.env (top level, easy to find)
    if [ ! -f "$MARCEL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MARCEL_HOME/.env"
            log_success "Created ~/.marcel/.env from template"
        else
            touch "$MARCEL_HOME/.env"
            log_success "Created ~/.marcel/.env"
        fi
    else
        log_info "~/.marcel/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$MARCEL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.marcel/config.yaml (top level, easy to find)
    if [ ! -f "$MARCEL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MARCEL_HOME/config.yaml"
            log_success "Created ~/.marcel/config.yaml from template"
        fi
    else
        log_info "~/.marcel/config.yaml already exists, keeping it"
    fi
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.marcel/"

    # Seed bundled skills into ~/.marcel/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `marcel update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'marcel update'." \
            > "$MARCEL_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $MARCEL_HOME/.no-bundled-skills"
        log_info "  Future 'marcel update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.marcel/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.marcel/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MARCEL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$MARCEL_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.marcel/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$MARCEL_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Marcel Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Marcel will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Marcel Agent browser tools — explicit browser override."
...
# install — the same closure `marcel update` installs
# (marcel_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `marcel update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `marcel tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$MARCEL_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Marcel' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$MARCEL_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'marcel tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `marcel tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `marcel computer-use install` on their own).
    ...
    # Same upstream installer `marcel computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Marcel install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Marcel'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'marcel tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: marcel computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'marcel setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Marcel to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MARCEL_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'marcel whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MARCEL_CMD="$(get_marcel_command_path)"
                $MARCEL_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'marcel whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'marcel gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MARCEL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MARCEL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: marcel gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: marcel gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $MARCEL_CMD gateway > "$MARCEL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.marcel/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: marcel gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: marcel gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.marcel-bootstrap-complete, which tells the Marcel
    ...
    local marker_path="$INSTALL_DIR/.marcel-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MARCEL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MARCEL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MARCEL_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}marcel${NC}              Start chatting"
    echo -e "   ${GREEN}marcel setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}marcel config${NC}       View/edit configuration"
    echo -e "   ${GREEN}marcel config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}marcel gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}marcel update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'marcel' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'marcel' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'marcel' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$MARCEL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$MARCEL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$MARCEL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `marcel update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$MARCEL_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Marcel` rename
...
# _purge_electron_build_cache() used by `marcel desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Marcel-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `marcel desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Marcel" ]; then
            app="$desktop_dir/release/linux-unpacked/Marcel"
        elif [ -x "$desktop_dir/release/linux-unpacked/marcel" ]; then
            app="$desktop_dir/release/linux-unpacked/marcel"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Marcel.app" \
            "$desktop_dir/release/mac/Marcel.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `marcel desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading MARCEL_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if MARCEL_HOME="$MARCEL_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from marcel_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `marcel update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Marcel-managed Node
            # provisioned during prerequisites/node-deps (at $MARCEL_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $MARCEL_HOME.
    # $MARCEL_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'marcel update' on this host install.
...
if [ "$MANIFEST_MODE" = true ]; then
    ...
    # Shared mandatory precondition for main, ensure, and every mutating stage.
    migrate_legacy_home_before_install || exit 1
