...
unprivileged `marcel` user, giving the layout most people have —
$MARCEL_HOME/marcel-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  MARCEL_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .marcel-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `marcel setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${MARCEL_DEV_SANDBOX_UPSTREAM:-https://github.com/AdMind-ai/marcel-agent.git}"
...
GIT_ROOT="${MARCEL_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${MARCEL_DEV_SANDBOX_DIR:-.marcel-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t marcel-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t marcel-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/marcel-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `marcel` launcher to ~/.local/bin and the
# checkout to $MARCEL_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/marcel-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q marcel-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t marcel-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Marcel sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/marcel-sandbox/next"
