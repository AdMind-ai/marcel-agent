# ============================================================================
# Marcel Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$MarcelHome = $(if ($env:MARCEL_HOME) { $env:MARCEL_HOME } else { "$env:LOCALAPPDATA\Marcel" }),
    [string]$InstallDir = $(if ($env:MARCEL_HOME) { "$env:MARCEL_HOME\marcel-agent" } else { "$env:LOCALAPPDATA\Marcel\marcel-agent" }),
    ...
    # builds apps/desktop into a launchable Marcel.exe.
    #
    # Why opt-in:
    #   * Marcel-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Marcel.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Marcel.exe
    ...
    #     `marcel desktop` already builds on demand.
...
# MARCEL_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[marcel] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'MarcelInstall.LongPath').Type) {
            Add-Type -Namespace 'MarcelInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [MarcelInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -MarcelHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('MarcelHome')) {
    $MarcelHome = ConvertTo-LongPath $MarcelHome
} else {
    $MarcelHome = ConvertTo-LongPath $(
        if ($env:MARCEL_HOME) { $env:MARCEL_HOME } else { "$env:LOCALAPPDATA\Marcel" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:MARCEL_HOME) { "$env:MARCEL_HOME\marcel-agent" } else { "$env:LOCALAPPDATA\Marcel\marcel-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: MarcelHome=$MarcelHome InstallDir=$InstallDir"
}

# Cut over legacy data before any install stage can scaffold MarcelHome.
function Invoke-LegacyMigrationPrecondition {
    $migrationModule = Join-Path $PSScriptRoot "..\marcel_migration.py"
    if (-not (Test-Path $migrationModule)) {
        throw "Marcel migration module is unavailable; refusing to create MARCEL_HOME."
    }
    $migrationRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
    $python = Get-Command python -ErrorAction SilentlyContinue
    if (-not $python) { $python = Get-Command python3 -ErrorAction SilentlyContinue }
    if (-not $python) {
        throw "Python is required for the pre-install legacy migration."
    }
    $previousPythonPath = $env:PYTHONPATH
    $env:PYTHONPATH = "$migrationRoot$(if ($previousPythonPath) { ";$previousPythonPath" })"
    & $python.Source -m marcel_migration --destination $MarcelHome
    if ($LASTEXITCODE -ne 0) {
        throw "Legacy Marcel home migration failed; refusing to continue installation."
    }
...
$RepoUrlSsh = "git@github.com:AdMind-ai/marcel-agent.git"
$RepoUrlHttps = "https://github.com/AdMind-ai/marcel-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|                * Marcel Installer                       |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An autonomous AI agent framework by AdMind.            |" -ForegroundColor Magenta
    Write-Host "|  Upstream: Nous Research.                               |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MarcelHome)) {
        New-Item -ItemType Directory -Force -Path $MarcelHome | Out-Null
    }
    $envFile = Join-Path $MarcelHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $MarcelHome "node"
...
function Install-Uv {
    # Marcel owns its own uv at $MarcelHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (marcel_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `marcel update` stay in sync.
    $managedUv = Join-Path $MarcelHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $MarcelHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $MarcelHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $MarcelHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $MarcelHome "bin"
        ...
        # (marcel_cli/managed_uv.py looks only at $MarcelHome\bin\uv.exe).
...
# Put the Marcel-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone marcel-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Marcel-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Marcel-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("marcel-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and MARCEL_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Marcel belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # marcel_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".marcel-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Marcel-managed interpreter
    ...
    #
    # Under Marcel-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Marcel-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Marcel setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Marcel uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\marcel\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Marcel needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Marcel copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\marcel\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MARCEL_GIT_BASH_PATH`` (User scope) so Marcel can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Marcel-managed PortableGit install instead..."
    }

    # Download PortableGit into $MarcelHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MarcelHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Marcel features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Marcel needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MARCEL_GIT_BASH_PATH`` (User env scope) so Marcel can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MarcelHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MarcelHome\git\usr\bin\bash.exe
    $candidates += "$MarcelHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MarcelHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MARCEL_GIT_BASH_PATH", $candidate, "User")
            $env:MARCEL_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set MARCEL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Marcel may not find Git Bash."
    Write-Info "If needed, set MARCEL_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Marcel-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Marcel requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Marcel requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Marcel-managed Node.js installation instead..."

    # Prefer a Marcel-managed Node from a previous run over a too-old system one.
    $managedNode = "$MarcelHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$MarcelHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$MarcelHome\node"
        Write-Success "Node.js $version found (Marcel-managed)"
        ...
        Update-ManagedNpm "$MarcelHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Marcel-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $MarcelHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MarcelHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\marcel-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$MarcelHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$MarcelHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$MarcelHome\node") {
                    try {
                        Rename-Item "$MarcelHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Marcel-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$MarcelHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MarcelHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\marcel-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `marcel
                    # update` path (#4735).
                    ...
                    $stashName = "marcel-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. marcel-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``marcel update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Marcel behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running marcel processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/AdMind-ai/marcel-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/AdMind-ai/marcel-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/AdMind-ai/marcel-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\marcel-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\marcel-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `marcel update` (see the notes at the shared clone-path
    ...
    # next `marcel update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Marcel-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Marcel-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running marcel process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running marcel processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Marcel_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (marcel.exe) plus its child tree.
            & taskkill /F /T /IM marcel.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM marcel.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m marcel_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not marcel.exe.
            ...
            # Marcel children re-exec through .marcel-runtime, so killing only
            ...
            # user (see marcel_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Marcel " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `marcel update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Marcel (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `marcel.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install marcel-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and marcel.exe broken with
    ...
    # here, not 30 seconds later when the user runs `marcel`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Marcel processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Marcel processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Marcel and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register marcel.exe in dist-info/RECORD but fail to
        ...
        # `marcel` command while marcel-agent.exe / marcel-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m marcel_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `marcel dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\marcel_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `marcel dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `marcel dashboard` should now work."
            } else {
...
function Install-MarcelCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the marcel launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring marcel.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "marcel.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the marcel command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # marcel_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("marcel", "marcel-acp")) {
    ...
    $requiredExe = Join-Path $Destination "marcel.exe"
    $requiredCmd = Join-Path $Destination "marcel.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the marcel command: launcher was not installed: $requiredExe"
    }
    return $Destination
}
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        # $MarcelHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `marcel update`'s autostash
        ...
        # installer staged under marcel-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-MarcelCommandLaunchers, which throws BEFORE any PATH
    ...
    #   marcel-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The marcel-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if ($currentPath -notlike "*$marcelBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$marcelBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $marcelBin"
    } else {
    ...
    $currentMarcelHome = [Environment]::GetEnvironmentVariable("MARCEL_HOME", "User")
    if (-not $currentMarcelHome -or $currentMarcelHome -ne $MarcelHome) {
        [Environment]::SetEnvironmentVariable("MARCEL_HOME", $MarcelHome, "User")
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.marcel-bootstrap-complete which tells the Marcel
    ...
    # Marcel-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `marcel update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".marcel-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$MarcelHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarcelHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarcelHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarcelHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MarcelHome\audio_cache" | Out-Null
    ...
    # (the default PS5 behaviour), and Marcel's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$MarcelHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # This exact AdMind-branded installer template is registered as a safe
        # legacy template in marcel_cli/default_soul.py. The setup wizard can
...
"@
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $MarcelHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MarcelHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $MarcelHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Marcel-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'marcel setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\marcel-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\marcel-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/AdMind-ai/marcel-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
